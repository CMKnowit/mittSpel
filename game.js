...
if (!lowestJumpCountElement) {
  lowestJumpCountElement = document.createElement("span");
}
if (!currentJumpCountElement) {
  currentJumpCountElement = document.createElement("span");
}
...
function resetGame() {
  ...
  if (
    gamestate.Player.isWinning &&
    gamestate.ScoreKeeping.currentJumpCount > 0 &&
    gamestate.ScoreKeeping.currentJumpCount <
      gamestate.ScoreKeeping.lowestJumpCount
  ) {
    gamestate.ScoreKeeping.lowestJumpCount =
      gamestate.ScoreKeeping.currentJumpCount;
  }
  gamestate.ScoreKeeping.currentJumpCount = 0;
  lowestJumpCountElement.innerText =
    "Highscore (fewest jumps to reach golden platform): " +
    gamestate.ScoreKeeping.lowestJumpCount;
...
function update() {
  ...
  // Update gamestate.Player position
  gamestate.Player.x += gamestate.Player.xSpeed;
  gamestate.Player.y += gamestate.Player.ySpeed;

  // Keep the gamestate.Player within the canvasElement bounds
...
// icy tower style logic: move the view upwards with ctx.translate(0,100);
